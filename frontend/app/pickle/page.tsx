...
// Contador de cliques para desbloquear segredos
...
export default function PicklePage() {
  ...
  // Efeito para configurar a página
  useEffect(() => {
    // Adicionar classe pickle-mode ao body
    ...
    const visitCount = Number.parseInt(localStorage.getItem("pickleVisits") || "0") + 1
    ...
    if (visitCount > 1) {
      toast({
        ...
        description:
          visitCount >= 5 ? "Você realmente gosta de pepinos, hein?" : "Continue voltando para mais surpresas!",
        duration: 3000,
      })
    }

    // Criar elemento de áudio programaticamente
    const audio = new Audio("/pickle-rick-sound.mp3")
    audio.loop = true
    audio.volume = 0.7
    audioRef.current = audio

    // Evento para quando o áudio estiver carregado
  ...
  // Função para alternar áudio com tratamento de erros
  const toggleAudio = () => {
    try {
      if (!audioRef.current || !audioLoaded) {
        ...
        toast({
          title: "Ops!",
          description: "O áudio está carregando, tente novamente em alguns segundos.",
          duration: 3000,
        })
        return
      }
  ...
  // Função para lidar com cliques no Pickle Rick
  const handlePickleClick = () => {
    ...
    // Mudar a frase
    setCurrentQuote(PICKLE_QUOTES[Math.floor(Math.random() * PICKLE_QUOTES.length)])

    // Verificar se desbloqueou algo
    if (newCount === CLICK_THRESHOLDS.VOICE) {
      ...
      // Tentar ativar o áudio, mas não forçar
      if (audioRef.current && audioLoaded) {
        const playPromise = audioRef.current.play()
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch((e) => {
              console.warn("Não foi possível reproduzir áudio automaticamente:", e)
            })
        }
      }
  ...
  const startGame = () => {
    ...
    // Posicionar o pickle aleatoriamente
    movePickleRandomly()
  ...
  // Clicar no pickle durante o jogo
  const catchPickle = () => {
  ...
  return (
    <main className="container mx-auto px-4 py-16 text-center relative min-h-[80vh]">
      ...
      {/* Contador de cliques (secreto) */}
      <div className="absolute top-4 left-4 text-xs text-muted-foreground">Pickle Power: {clickCount}</div>
      ...
      {!showGame && !showUltimatePickle && (
        <>
          <motion.div
            ...
            whileTap={{ scale: 0.9 }}
          >
            <Image src="/pickle-rick.png" alt="Pickle Rick" width={300} height={400} className="mx-auto" priority />
          </motion.div>
      ...
      {showGame && (
        <div className="mb-8">
          ...
          {!gameActive ? (
            <div className="mb-6">
              <p className="mb-4">Clique no Pickle Rick o máximo de vezes que conseguir em 30 segundos!</p>
      ...
      {/* Ultimate Pickle (desbloqueado com muitos cliques) */}
      {showUltimatePickle && (
        <div className="mb-8">
          <motion.div
          ...
          >
            <Image src="/pickle-rick.png" alt="Ultimate Pickle Rick" width={400} height={500} className="mx-auto" />
            <motion.div
